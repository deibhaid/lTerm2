...
### Goals
- Provide a Linux-first UI while preserving iTerm2’s UX patterns (tabs, splits, profiles, triggers, search, status bar).
...
| `automation/` | DBus/gRPC surface mirroring iTerm2 scripting | Eventually supports triggers, bookmarks, scripts. |
...
3. **Feature parity** – iterate through iTerm2 features (search, triggers, tmux, status bar) and port UI + glue.
