...
Purpose: capture the concrete steps needed to move the Objective-C parser/terminal stack into `liblterm-core` while keeping behavior parity with the macOS upstream terminal.

### 1. Modules to Extract
| Module | Source files | Notes |
| --- | --- | --- |
| Token data structures | `VT100Token.{h,m}`, `VT100CSIParam.{h,m}` | Convert Objective-C classes into C structs (`lterm_token`, `lterm_csi_param`). **Done:** `lterm_token_types.h`, `lterm_csi_param.h`, `lterm_screen_char.h`, `lterm_token.h/.c`.
...
core/
  include/
    lterm_token.h        # C structs/enums for tokens
    lterm_terminal.h     # Terminal instance + delegate vtable
  src/parser/
    vt100_token.c
    vt100_parser.c
    vt100_terminal.c
...
- **Strings/Unicode**: replace `NSString`/`NSData` with UTF-8 byte arrays; limit conversions to portability-friendly helpers (`lterm_unicode_utils`).
...
- **Delegates**: define `struct lterm_terminal_delegate` with callbacks:
...
- **Byte stream**: `lterm_reader` (new) mirrors `VT100ByteStream` to feed the parser without Objective-C dependencies.
